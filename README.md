...
- `/F`: Text files containing the components of the polynomial vector $\vec{F}$, which generates the kernel of the Hessian matrix. To reduce file sizes, these components are saved in factored form.
- `/graphs`: Text files providing edge lists (vertex set $\\{0, 1, 2, 3, 4, 5, 6, 7\\}$).
- `/H`: CSV files of the higher Hessian matrices $H=H_{B_3}(f_G)$.
...
- `verify.sage`: A SageMath script to re-verify the algebraic relations ($H\vec{F} = \vec{0}$, etc.) and data consistency.
...
- `deg_f_1`, `deg_f_2`: Degrees of the polynomial components in the kernel vector $\vec{F}$. 
- `edge_set_binary`: 28-bit binary string representing the edges (subset of all pairs in $\\{0, 1, 2, 3, 4, 5, 6, 7\\}$).
...
This script will check the consistency between the various graph representations, re-calculate $f_G$ and $H$ from the matroid structure and the basis $B_3$, and confirm that $H\vec{F} = \vec{0}$.

> [!WARNING]
> `verify.sage` requires approximately 6 GB of memory. 
